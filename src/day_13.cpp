...
// the first bus ID departs at that timeand each subsequent listed bus ID departs at that subsequent minute. (The first
...
struct Num_Info {
    uint64_t number;
    uint64_t distance_to_last_number;
};
...
std::string day_13_b(char const * input_file_path)
{
    ...
    uint64_t distance{};
    auto safe_increment{ num_infos.front().number };

    for (auto num_info{ num_infos.cbegin() + 1 }; num_info != num_infos.cend(); ++num_info) {
        distance += num_info->distance_to_last_number;
        auto const target_reminder{ num_info->number - distance % num_info->number };
        while (current_candidate % num_info->number != target_reminder) {
            current_candidate += safe_increment;
        }
        safe_increment *= num_info->number;
    }

    return std::to_string(current_candidate);
}